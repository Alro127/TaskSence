...
public class WorkspaceJoinRequestController {
    ...
    public ResponseEntity<ApiResponse<List<WorkspaceJoinRequestResponse>>> getWorkspaceJoinRequests(
            @PathVariable Long workspaceId
    ) {

        ApiResponse<List<WorkspaceJoinRequestResponse>> response = new ApiResponse<>(
                "200",
                "Get workspace join requests successfully",
                joinRequestService.getWorkspaceJoinRequests(workspaceId),
                null
        );
