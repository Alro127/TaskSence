...
| 2026-04-14 | AI Action Execution | Mutation-style requests are delegated from AI service to Spring MCP endpoint scaffold. | Keeps write-side authority in backend domain service and allows AI module to focus on planning/organization. |
...
| 2026-04-15 | Agent Mode Contract | Frontend explicitly sends `agent` boolean in `/api/v1/ai/chat/{session_id}` payload and AI pipeline uses it to run action-agent-first logic. | Makes agent behavior deterministic, user-controlled at UI level, and backward-compatible when disabled. |
...
| 2026-05-15 | AI Maintainability | Keep `app.service.source_truth_service` as a compatibility facade while new source-truth code lives under `app.service.source_truth.*`. | Allows incremental module splits without breaking existing chatbots, embedding, or helper imports. |
